...
export default function NovelImportPage() {
  ...
  const invalidNovelId = Boolean(username && !novelId);
  const visibleNovelError = novelError || (invalidNovelId ? '小说项目不存在' : '');
  ...
  if (isLoading || (novelLoading && !invalidNovelId)) {
    return (
  ...
  if (visibleNovelError) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center">
        <p className="text-error text-sm mb-4">{visibleNovelError}</p>

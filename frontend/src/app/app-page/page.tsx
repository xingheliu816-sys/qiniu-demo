...
export default function AppPage() {
  ...
  async function handleParse() {
    ...
    try {
      ...
      const data = await api.parseChapters(title, content, detectedType, chapterName);
      if (!data.success) {
  ...
  function handleClear() {
    if (!confirm('确定要清空小说标题和正文内容吗？此操作不可恢复。')) return;
    setTitle('');
    setChapterName('');
    setContent('');
    setFileName('');
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              {fileName && (
                <span className="text-xs text-ink-light max-w-[180px] truncate">{fileName}</span>
              )}
            </div>
              ...
              <div className="px-6 pb-5 space-y-2">
                {result.chapters.map((ch) => (
                  <div key={ch.index} className="border border-border rounded-lg overflow-hidden">
                    <div
                      className="flex items-center justify-between px-4 py-2.5 bg-paper/50 hover:bg-paper cursor-pointer transition-colors"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 shrink-0 ml-3">
                        ...
                        <span className="text-xs text-accent font-medium">
                          {expandedChapters.has(ch.index) ? '收起' : '展开'}
                        </span>
                      </div>
                    </div>
